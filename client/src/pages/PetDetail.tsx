...
import AdoptionAIChat from '@/components/AdoptionAIChat';
import ImageSlider from '@/components/ImageSlider';
import { usePet } from '@/hooks/usePets';
import { adoptionsApi } from '@/lib/api/adoptions.api';
import { getErrorMessage } from '@/lib/errorHandler';
import { ArrowLeft, CheckCircle2, Home, MapPin, Sparkles } from 'lucide-react';
...
// ── Shared input styles ───────────────────────────────────────────────────────
const inp =
  'w-full px-4 py-2.5 border rounded-xl bg-white text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition-shadow text-sm';
...
export default function PetDetail() {
  ...
  const onSubmit = async (data: ApplicationForm) => {
    if (!petId) return;
    try {
      const homeType = data.homeType as
        | 'house'
        | 'apartment'
        | 'condo'
        | 'townhouse'
          ...
          <div className="grid md:grid-cols-2 gap-12">
            ...
            <div className="space-y-4 pt-2">
              {[3, 1.5, 1, 1, 0.75].map((w, i) => (
                <div
                  key={i}
                  className={`h-${i === 0 ? 10 : 5} rounded-xl bg-gray-100 animate-pulse`}
                  style={{ width: `${w * 33}%` }}
                />
              ))}
            </div>
          ...
          <div className="flex flex-col gap-6">
              ...
              {[
                ...
                pet.adoptionFee
                  ? {
                      ...
                      cls: 'text-amber-600 font-bold text-xl',
                    }
                  : {
                      label: 'Adoption Fee',
                      value: 'Free to adopt',
                ...
                >
                  ...
                  <p className={`font-semibold text-gray-900 ${cls ?? ''}`}>
                    {value}
                  </p>
                </div>
              ))}
            ...
            {/* {submitted ? (
              ...
              !showForm ? (
                <button
                  onClick={() =>
                    isAuthenticated ? setShowForm(true) : setLocation('/login')
                  }
            ...
            {submitted ? (
              ...
              !showForm && !showAIChat ? (
                ...
                // ── Cancel button when either is open ──────────────────
                <button
                  onClick={() => {
                    setShowForm(false);
                    setShowAIChat(false);
        ...
        {showAIChat && isAvailable && (
          <AdoptionAIChat
            ...
            onSubmitSuccess={() => {
              setShowAIChat(false);
              setSubmitted(true);
              // trigger whatever you do after successful submission
            }}
            onSubmit={onSubmit}
          />
        )}
          ...
          <div className="mt-12 bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="bg-amber-500 px-8 py-6">
              ...
              <p className="text-amber-100 text-sm mt-1">
                Applying to adopt <span className="font-bold">{pet.name}</span>
              </p>
            </div>
            ...
            >
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className={errors.fullName ? inpErr : inpOk}
                  />
                  {errors.fullName && (
                ...
                <div>
                  ...
                  <input
                    ...
                    className={errors.address ? inpErr : inpOk}
                  />
                  {errors.address && (
                    <p className="text-red-500 text-xs mt-1">
                ...
                <label className="flex items-center gap-3 cursor-pointer pb-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      Check if your home has outdoor area
                    </p>
                  </div>
                </label>
              </div>
              ...
              <div>
                ...
                <textarea
                  {...register('reason', {
                    required:
                      'Please explain why you want to adopt (min 20 chars)',
                    minLength: {
                      value: 20,
                      message:
                        'Please explain why you want to adopt (min 20 chars)',
